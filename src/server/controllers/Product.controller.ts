...
export default class ProductController {
            ...
            fs.remove(
              ...
              async () => {
                ...
                if (productToDelete) {
                  ...
                  this.throwError(
                    'Something went wrong, during deleting the product.',
                    500,
                    next
                  );
                }
  ...
  public async store(
    ...
    try {
      const productItem: IProduct = await Products.findOne({
      ...
      if (productItem) {
        ...
        if (req.body.imageFilesData && req.body.imageFilesData.length > 0) {
          const folderName: string = req.body.title
            .toLowerCase()
            .replace(/\s+/g, '-');
          ...
          fs.mkdir(path.resolve(__dirname + folderPath), (err) => {
            ...
            for (const imageData of req.body.imageFilesData) {
              ...
              fs.writeFile(
                __dirname + folderPath + '/' + uID + '.' + imageData.type,
                base64Data,
                { encoding: 'base64' },
                (fileErr) => {
                  if (fileErr) {
                    ...
                    if (jnum === req.body.imageFilesData.length) {
                      ...
                      const asyncCreate = async (product: IProduct) => {
                        try {
                          const createProduct: object = await Products.create(
                            product
                          );
